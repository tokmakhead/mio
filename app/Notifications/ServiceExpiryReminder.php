...
class ServiceExpiryReminder extends Notification implements ShouldQueue
{
    ...
    public function toMail(object $notifiable): MailMessage
    {
        return (new MailMessage)
            ->subject('Hizmet Yenileme Hatırlatması - ' . $this->service->name)
            ->greeting('Merhaba ' . $notifiable->name . ',')
            ->line('"' . $this->service->name . '" hizmetinizin süresi ' . $this->daysLeft . ' gün sonra dolacaktır.')
            ->line('Son Kullanma Tarihi: ' . $this->service->end_date->format('d.m.Y'))
