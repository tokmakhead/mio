    ...
    <div class="py-8">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                ...
                @php $summaryLimit = 3;
                $summaryCount = 0; @endphp

                @foreach($globalSummary as $currency => $amounts)
                    {{-- Show Receivables (Alacak) --}}
                    @if($amounts['receivable'] > 0 && $summaryCount < $summaryLimit)
                    ...
                    {{-- Show Payables (Borç) --}}
                    @if($amounts['payable'] > 0 && $summaryCount < $summaryLimit)
                        @php $summaryCount++; @endphp
                        <x-kpi-card title="Borç ({{ $currency }})"
                            value="{{ number_format($amounts['payable'], 2, ',', '.') }}{{ $currency == 'TRY' ? '₺' : ' ' . $currency }}"
                            tone="danger"
                ...
                {{-- Always fill the rest with "Aktif Müşteri" if we have slots left --}}
                @while($summaryCount < $summaryLimit)
