    ...
    <x-page-banner title="Gelir Analizi" subtitle="Dönemsel kesilen fatura, tahsilat ve bekleyen tutar analizi.">
        <x-slot name="actions">
            <div class="flex space-x-2">
                <a href="{{ route('reports.revenue.csv', ['period' => $period]) }}"
                ...
                <a href="{{ route('reports.revenue.pdf', ['period' => $period]) }}"
                    class="inline-flex items-center px-4 py-2 bg-white/10 hover:bg-white/20 text-white font-semibold rounded-lg backdrop-blur-sm transition-all">
                    <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
                            d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
            ...
            <div class="flex justify-center space-x-4">
                @foreach([1, 3, 6, 12] as $p)
                    <a href="{{ route('reports.revenue', ['period' => $p]) }}"
                        class="px-6 py-2 rounded-full font-semibold transition-all {{ $period == $p ? 'bg-primary-600 text-white shadow-lg shadow-primary-500/30' : 'bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700' }}">
                        Son {{ $p }} Ay
                    </a>
                @endforeach
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                <x-kpi-card title="Toplam Kesilen" value="{{ number_format($totalInvoiced, 2) }} ₺" tone="primary"
                ...
                <x-kpi-card title="Toplam Tahsilat" value="{{ number_format($totalCollected, 2) }} ₺" tone="success"
                ...
                <x-kpi-card title="Toplam Bekleyen" value="{{ number_format($totalPending, 2) }} ₺" tone="warning"
                ...
                <x-kpi-card title="Ort. Fatura Tutarı" value="{{ number_format($averageInvoice, 2) }} ₺" tone="indigo"
            ...
            <!-- Table -->
            <x-card>
                <h3 class="text-lg font-bold text-gray-900 dark:text-gray-100 mb-6">Aylık Döküm</h3>
